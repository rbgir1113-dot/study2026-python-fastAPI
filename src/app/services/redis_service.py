...
class RedisService:
  ...
  # Redis Stack에 refresh token을 저장
  async def save_refresh_token(self, member_id: int, refresh_token: str):
    try:
  ...
  # Redis Stack에 등록된 token 삭제
  async def delete_refresh_token(self, member_id) -> None:
    key = f"{REFRESH_TOKEN_PREFIX}{member_id}"
    await self.repo.delete(key)


  # Redis Stack에 블랙리스트에 token 등록
