...
from app.schemas.member_schema import (
  MemberCreateDTO, MemberClaimsDTO, MemberUpdateDTO, MemberResponseDTO, SocialMemberCreateDTO
)
...
class MemberRepository:
  ...
  # 회원가입
  async def create_member(self, member: MemberCreateDTO) -> MemberCreateDTO:
    # 1) Query 직접 작성
    new_member = {
  ...
  # 소셜 회원가입
  async def create_socail_member(self, member: MemberCreateDTO) -> MemberCreateDTO:
    # 1) Query 직접 작성
    new_member = {
      "member_email": member.member_email,
      "member_name": member.member_name,
  ...
  async def find_member_by_id(self, id: int) -> Member | None:
    ...
    # print("found_member", found_member)
    # return found_member
  ...
  async def update_member(self, id: int, member:MemberUpdateDTO)-> bool:
    # 1) 직접 작성
    # result: int = await self.db.execute(UPDATE_MEMBER_QUERY, {
    #   "id" :id,
    #   "member_password" : member.member_password,
    #   "member_name" : member.member_name,
    #   "member_age" : member.member_age
